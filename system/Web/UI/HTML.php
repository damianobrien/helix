...
class HTML extends Object {
	...
	public function cache() {
		...
		if (OutputBuffer::isActive()) {
			...
			file_put_contents("{$config["root"]}/cache/{$key}.html", OutputBuffer::getContents());
		} */
	}
	...
	/**
	 * Add scripts to be used in this HTMLDocument 
	 * 
	 * @param bool $externalFirst to include external links first, or inline first.
	 *  Toggle to meet dependency needs
	 */
	public function insertScripts($externalFirst=true) {
		...
		if (is_array($this->styles)) {
			if (isset($this->styles["external"]) && is_array($this->styles["external"])) {
				foreach ($this->styles["external"] as $url) {
					echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"{$url}\"/>" . NL;
				}
			}
