 ...
 * 
 * This class represents the board table in the Helix database schema.
 * Use this class to select, insert, update and delete data in the board
 * table, as well as access related data in other tables.
 * 
 * If you need to extend the functionality of this class, code should be placed in a
 * class called BoardExtension, and should extend the BoardTable
 * class.  The custom code file should be in the helix/modules/Forum folder
 * and should be called BoardExtension.php
 * 
 * Object -> Board
 */
class BoardTable extends Object {
	public $logSequence;
	public $fdate;
	public $tdate;
	...
	public function __construct($id=null) {
		...
		if (isset($id)) {
			$condition = "{$db->le}board{$db->re}.{$db->le}id{$db->re}={$db->queryValue($id)}";
		}

		if (isset($condition)) {
			$query = implode(NL, array(
				"SELECT {$db->le}board{$db->re}.{$db->le}id{$db->re}, {$db->le}board{$db->re}.{$db->le}name{$db->re}, {$db->le}board{$db->re}.{$db->le}description{$db->re}, {$db->le}board{$db->re}.{$db->le}views{$db->re}, {$db->le}board{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}board{$db->re}.{$db->le}mdate{$db->re}, {$db->le}board{$db->re}.{$db->le}cdate{$db->re}, {$db->le}board{$db->re}.{$db->le}deleted{$db->re}",
	...
	public static function snapshot($date, $id=null) {
		...
		if (isset($condition)) {
			$condition .= " AND ({$db->le}board{$db->re}.{$db->le}tdate{$db->re} IS NOT NULL AND {$db->le}board{$db->re}.{$db->le}fdate{$db->re}<={$db->queryValue($date)} AND {$db->queryValue($date)}<={$db->le}board{$db->re}.{$db->le}tdate{$db->re}) ";
			$query = implode(NL, array(
				"SELECT {$db->le}board{$db->re}.{$db->le}log_sequence{$db->re}, {$db->le}board{$db->re}.{$db->le}fdate{$db->re}, {$db->le}board{$db->re}.{$db->le}tdate{$db->re}, {$db->le}board{$db->re}.{$db->le}id{$db->re}, {$db->le}board{$db->re}.{$db->le}name{$db->re}, {$db->le}board{$db->re}.{$db->le}description{$db->re}, {$db->le}board{$db->re}.{$db->le}views{$db->re}, {$db->le}board{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}board{$db->re}.{$db->le}mdate{$db->re}, {$db->le}board{$db->re}.{$db->le}cdate{$db->re}, {$db->le}board{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}board{$db->re}",
				"WHERE {$condition}"
			));

			$query .= "ORDER BY {$db->le}board{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	public function purge() {
		if (isset($this->_snapshot)) {return false;}
		$db = Database::getInstance();
		$query = "DELETE FROM " . $db->encapsulate("board") . " WHERE id={$db->queryValue($this->id)}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function deleteAll($where=null) {
		...
		$query = "UPDATE " . $db->encapsulate("board") . " SET deleted=1 {$condition}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function select($columns, $order=null, $where=null, $limit=null, $offset=0) {
		...
		$query = implode(NL, array(
			...
			"FROM {$db->le}board{$db->le}",

			"WHERE {$db->le}board{$db->re}.{$db->le}deleted{$db->re}=0" . (isset($where) ? " AND ({$where})" : ""),
	...
	public static function ids($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		return Board::select("{$db->le}board{$db->le}.id", $order, $where, $limit, $offset);
	}

	public static function objects($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		$objects = array();
		foreach (Board::select("{$db->le}board{$db->re}.{$db->le}id{$db->re}, {$db->le}board{$db->re}.{$db->le}name{$db->re}, {$db->le}board{$db->re}.{$db->le}description{$db->re}, {$db->le}board{$db->re}.{$db->le}views{$db->re}, {$db->le}board{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}board{$db->re}.{$db->le}mdate{$db->re}, {$db->le}board{$db->re}.{$db->le}cdate{$db->re}, {$db->le}board{$db->re}.{$db->le}deleted{$db->re}", $order, $where, $limit, $offset) as $record) {
			$object = new Board();
				$object->id = $record["id"];
	...
	public static function search($query=null, $order=null, $where=null, $limit=null, $offset=0) {
		...
		foreach ($keywords as $keyword) {
			$clauses[] = "board.name LIKE '%{$keyword}%' OR board.description LIKE '%{$keyword}%'";
		}

		$search = implode(" AND ", $clauses);
		$where = isset($where) ? "{$where} AND ({$search})" : "({$search})";
		return Board::objects($order, $where, $limit, $offset);
	}

	public function __toString() {
		return "Board Object [" . alt($this->id, "null") . "]";
	}
	...
	public function getBoardTopic($topicId, $type="default") {
		return new BoardTopic(null, $this->id, $topicId, BoardTopic::typeId($type));
	}
	...
	public function removeTopicList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return BoardTopic::deleteAll($this->id, null, $type);
		}
	}
	...
	public function getTopicIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}board_topic{$db->re} ON {$db->le}board_topic{$db->re}.{$db->le}topic_id{$db->re}={$db->le}topic{$db->re}.{$db->le}id{$db->re} ",
			"	AND {$db->le}board_topic{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}topic{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}board_topic{$db->re}.{$db->le}board_id{$db->re}={$db->queryValue($this->id)} ",
