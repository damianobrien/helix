 ...
 * 
 * This class represents the prospect table in the Helix database schema.
 * Use this class to select, insert, update and delete data in the prospect
 * table, as well as access related data in other tables.
 * 
 * If you need to extend the functionality of this class, code should be placed in a
 * class called ProspectExtension, and should extend the ProspectTable
 * class.  The custom code file should be in the helix/modules/Project folder
 * and should be called ProspectExtension.php
 * 
 * Object -> Projectentity -> Prospect
 */
class ProspectTable extends Projectentity {
	public $logSequence;
	public $fdate;
	public $tdate;
	...
	public function __construct($id=null, $name=null, $code=null) {
		...
		if (isset($id)) {
			$condition = "{$db->le}prospect{$db->re}.{$db->le}projectentity_id{$db->re}={$db->queryValue($id)}";
		} else if (isset($name)) {
		...
		if (isset($condition)) {
			$query = implode(NL, array(
				"SELECT {$db->le}projectentity{$db->re}.{$db->le}id{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}name{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}code{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}description{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}projectentitystatus_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}mdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}cdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}prospect{$db->re}",
			"INNER JOIN {$db->le}projectentity{$db->re} ON {$db->le}prospect{$db->re}.{$db->le}projectentity_id{$db->re}={$db->le}projectentity{$db->re}.{$db->le}id{$db->re} AND {$condition}"
			));
	...
	public static function snapshot($date, $id=null, $name=null, $code=null) {
		...
		if (isset($condition)) {
			$condition .= " AND ({$db->le}prospect{$db->re}.{$db->le}tdate{$db->re} IS NOT NULL AND {$db->le}prospect{$db->re}.{$db->le}fdate{$db->re}<={$db->queryValue($date)} AND {$db->queryValue($date)}<={$db->le}prospect{$db->re}.{$db->le}tdate{$db->re}) ";
			...
			$query = implode(NL, array(
				"SELECT {$db->le}prospect{$db->re}.{$db->le}log_sequence{$db->re}, {$db->le}prospect{$db->re}.{$db->le}fdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}tdate{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}id{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}name{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}code{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}description{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}projectentitystatus_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}mdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}cdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}prospect{$db->re}",
			"INNER JOIN {$db->le}projectentity{$db->re} ON {$db->le}prospect{$db->re}.{$db->le}projectentity_id{$db->re}={$db->le}projectentity{$db->re}.{$db->le}id{$db->re} AND {$condition}"
			));

			$query .= "ORDER BY {$db->le}prospect{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	public function insert($insertParent=true) {
		...
		$query = implode(NL, array(
			"INSERT INTO {$db->le}prospect{$db->re} (",
	...
	public function update() {
		...
		if ($this->isDirty()) {
			$db = Database::getInstance();
			$query = implode(NL, array(
				"UPDATE {$db->le}prospect{$db->re} SET",
	...
	protected function log() {
		...
		$db = Database::getInstance();
		$database = $db->getDatabase();
		$record = Prospect::select($db->encapsulate("prospect") . ".mdate as fdate,null as tdate,{$db->encapsulate("prospect")}.*",null,$db->encapsulate("prospect").".projectentity_id={$this->id}")->first();
		$log = "{$database}_log";
		...
		$query = implode(NL, array(
			"INSERT INTO {$dbLog->le}prospect{$dbLog->re} (",
			$insertColumns,
			") VALUES (",
			$insertValues,
			")"
		...
		if ($logSequence){
			$query = implode(NL, array(
				...
				"FROM {$dbLog->le}prospect{$dbLog->re}",
				"WHERE {$dbLog->le}prospect{$dbLog->re}.{$dbLog->le}projectentity_id{$dbLog->re}={$this->id}",
			...
			if ($dbLog->getRecord()) {
				...
				$query = implode(NL, array(
					"UPDATE {$dbLog->le}prospect{$dbLog->re}",
	...
	public function purge() {
		if (isset($this->_snapshot)) {return false;}
		$db = Database::getInstance();
		$query = "DELETE FROM " . $db->encapsulate("prospect") . " WHERE projectentity_id={$db->queryValue($this->id)}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function deleteAll($where=null) {
		...
		$query = "UPDATE " . $db->encapsulate("prospect") . " SET deleted=1 {$condition}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function select($columns, $order=null, $where=null, $limit=null, $offset=0) {
		...
		$query = implode(NL, array(
			...
			"FROM {$db->le}prospect{$db->le}",
			"INNER JOIN {$db->le}projectentity{$db->re} ON {$db->le}prospect{$db->re}.{$db->le}projectentity_id{$db->re}={$db->le}projectentity{$db->re}.{$db->le}id{$db->re}",
			...
			"WHERE {$db->le}prospect{$db->re}.{$db->le}deleted{$db->re}=0" . (isset($where) ? " AND ({$where})" : ""),
	...
	public static function ids($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		return Prospect::select("{$db->le}prospect{$db->le}.projectentity_id", $order, $where, $limit, $offset);
	}

	public static function objects($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		$objects = array();
		foreach (Prospect::select("{$db->le}projectentity{$db->re}.{$db->le}id{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}name{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}code{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}description{$db->re}, {$db->le}projectentity{$db->re}.{$db->le}projectentitystatus_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}prospect{$db->re}.{$db->le}mdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}cdate{$db->re}, {$db->le}prospect{$db->re}.{$db->le}deleted{$db->re}", $order, $where, $limit, $offset) as $record) {
			$object = new Prospect();
				$object->id = $record["id"];
	...
	public static function search($query=null, $order=null, $where=null, $limit=null, $offset=0) {
		...
		return Prospect::objects($order, $where, $limit, $offset);
	}

	public function __toString() {
		return "Prospect Object [" . alt($this->id, "null") . "]";
	}
