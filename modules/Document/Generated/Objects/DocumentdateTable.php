...
class DocumentdateTable extends Object {
	...
	public static function snapshot($date, $id=null) {
		...
		if (isset($condition)) {
			...
			$query .= "ORDER BY {$db->le}documentdate{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	protected function log() {
		...
		$db = Database::getInstance();
		$database = $db->getDatabase();
		$record = Documentdate::select($db->encapsulate("documentdate") . ".mdate as fdate,null as tdate,{$db->encapsulate("documentdate")}.*",null,$db->encapsulate("documentdate").".id={$this->id}")->first();
		$log = "{$database}_log";
		...
		$query = implode(NL, array(
			"INSERT INTO {$dbLog->le}documentdate{$dbLog->re} (",
			$insertColumns,
			") VALUES (",
			$insertValues,
			")"
		...
		if ($logSequence){
			$query = implode(NL, array(
				...
				"FROM {$dbLog->le}documentdate{$dbLog->re}",
				"WHERE {$dbLog->le}documentdate{$dbLog->re}.{$dbLog->le}id{$dbLog->re}={$this->id}",
			...
			if ($dbLog->getRecord()) {
				...
				$query = implode(NL, array(
					"UPDATE {$dbLog->le}documentdate{$dbLog->re}",
	...
	public function __toString() {
		return "Documentdate Object [" . alt($this->id, "null") . "]";
	}
