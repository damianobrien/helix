 ...
 * 
 * This class represents the perm table in the Helix database schema.
 * Use this class to select, insert, update and delete data in the perm
 * table, as well as access related data in other tables.
 * 
 * If you need to extend the functionality of this class, code should be placed in a
 * class called PermExtension, and should extend the PermTable
 * class.  The custom code file should be in the helix/modules/Core folder
 * and should be called PermExtension.php
 * 
 * Object -> Perm
 */
class PermTable extends Object {
	public $logSequence;
	public $fdate;
	public $tdate;
	...
	public function __construct($id=null, $name=null) {
		...
		if (isset($id)) {
			$condition = "{$db->le}perm{$db->re}.{$db->le}id{$db->re}={$db->queryValue($id)}";
		} else if (isset($name)) {
			$condition = "{$db->le}perm{$db->re}.{$db->le}name{$db->re}={$db->queryValue($name)}";
		}

		if (isset($condition)) {
			$query = implode(NL, array(
				"SELECT {$db->le}perm{$db->re}.{$db->le}id{$db->re}, {$db->le}perm{$db->re}.{$db->le}name{$db->re}, {$db->le}perm{$db->re}.{$db->le}description{$db->re}, {$db->le}perm{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}mdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}cdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}deleted{$db->re}",
	...
	public static function snapshot($date, $id=null, $name=null) {
		...
		if (isset($condition)) {
			$condition .= " AND ({$db->le}perm{$db->re}.{$db->le}tdate{$db->re} IS NOT NULL AND {$db->le}perm{$db->re}.{$db->le}fdate{$db->re}<={$db->queryValue($date)} AND {$db->queryValue($date)}<={$db->le}perm{$db->re}.{$db->le}tdate{$db->re}) ";
			$query = implode(NL, array(
				"SELECT {$db->le}perm{$db->re}.{$db->le}log_sequence{$db->re}, {$db->le}perm{$db->re}.{$db->le}fdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}tdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}id{$db->re}, {$db->le}perm{$db->re}.{$db->le}name{$db->re}, {$db->le}perm{$db->re}.{$db->le}description{$db->re}, {$db->le}perm{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}mdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}cdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}perm{$db->re}",
				"WHERE {$condition}"
			));

			$query .= "ORDER BY {$db->le}perm{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	protected function log() {
		...
		$db = Database::getInstance();
		$database = $db->getDatabase();
		$record = Perm::select($db->encapsulate("perm") . ".mdate as fdate,null as tdate,{$db->encapsulate("perm")}.*",null,$db->encapsulate("perm").".id={$this->id}")->first();
		$log = "{$database}_log";
		...
		$query = implode(NL, array(
			"INSERT INTO {$dbLog->le}perm{$dbLog->re} (",
			$insertColumns,
			") VALUES (",
			$insertValues,
			")"
		...
		if ($logSequence){
			$query = implode(NL, array(
				...
				"FROM {$dbLog->le}perm{$dbLog->re}",
				"WHERE {$dbLog->le}perm{$dbLog->re}.{$dbLog->le}id{$dbLog->re}={$this->id}",
	...
	public function purge() {
		if (isset($this->_snapshot)) {return false;}
		$db = Database::getInstance();
		$query = "DELETE FROM " . $db->encapsulate("perm") . " WHERE id={$db->queryValue($this->id)}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function deleteAll($where=null) {
		...
		$query = "UPDATE " . $db->encapsulate("perm") . " SET deleted=1 {$condition}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function select($columns, $order=null, $where=null, $limit=null, $offset=0) {
		...
		$query = implode(NL, array(
			...
			"FROM {$db->le}perm{$db->le}",

			"WHERE {$db->le}perm{$db->re}.{$db->le}deleted{$db->re}=0" . (isset($where) ? " AND ({$where})" : ""),
	...
	public static function ids($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		return Perm::select("{$db->le}perm{$db->le}.id", $order, $where, $limit, $offset);
	}

	public static function objects($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		$objects = array();
		foreach (Perm::select("{$db->le}perm{$db->re}.{$db->le}id{$db->re}, {$db->le}perm{$db->re}.{$db->le}name{$db->re}, {$db->le}perm{$db->re}.{$db->le}description{$db->re}, {$db->le}perm{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}perm{$db->re}.{$db->le}mdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}cdate{$db->re}, {$db->le}perm{$db->re}.{$db->le}deleted{$db->re}", $order, $where, $limit, $offset) as $record) {
			$object = new Perm();
				$object->id = $record["id"];
	...
	public static function search($query=null, $order=null, $where=null, $limit=null, $offset=0) {
		...
		foreach ($keywords as $keyword) {
			$clauses[] = "perm.name LIKE '%{$keyword}%' OR perm.description LIKE '%{$keyword}%'";
		}

		$search = implode(" AND ", $clauses);
		$where = isset($where) ? "{$where} AND ({$search})" : "({$search})";
		return Perm::objects($order, $where, $limit, $offset);
	}

	public function __toString() {
		return "Perm Object [" . alt($this->id, "null") . "]";
	}
	...
	public function getPagePerm($pageId, $type="default") {
		return new PagePerm(null, $pageId, $this->id, PagePerm::typeId($type));
	}

	public function getPermRole($roleId, $type="default") {
		return new PermRole(null, $this->id, $roleId, PermRole::typeId($type));
	}

	public function getPermUser($userId, $type="default") {
		return new PermUser(null, $this->id, $userId, PermUser::typeId($type));
	}
	...
	public function removePageList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return PagePerm::deleteAll(null, $this->id, $type);
		}
	}
	...
	public function getPageIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}page_perm{$db->re} ON {$db->le}page_perm{$db->re}.{$db->le}page_id{$db->re}={$db->le}page{$db->re}.{$db->le}id{$db->re} ",
			"	AND {$db->le}page_perm{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}page{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}page_perm{$db->re}.{$db->le}perm_id{$db->re}={$db->queryValue($this->id)} ",
	...
	public function removeRoleList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return PermRole::deleteAll($this->id, null, $type);
		}
	}
	...
	public function getRoleIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}perm_role{$db->re} ON {$db->le}perm_role{$db->re}.{$db->le}role_id{$db->re}={$db->le}role{$db->re}.{$db->le}id{$db->re} ",
			"	AND {$db->le}perm_role{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}role{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}perm_role{$db->re}.{$db->le}perm_id{$db->re}={$db->queryValue($this->id)} ",
	...
	public function removeUserList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return PermUser::deleteAll($this->id, null, $type);
		}
	}
	...
	public function getUserIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}perm_user{$db->re} ON {$db->le}perm_user{$db->re}.{$db->le}user_person_entity_id{$db->re}={$db->le}user{$db->re}.{$db->le}person_entity_id{$db->re} ",
			"	AND {$db->le}perm_user{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}user{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}perm_user{$db->re}.{$db->le}perm_id{$db->re}={$db->queryValue($this->id)} ",
