 ...
 * 
 * This class represents the persondate table in the Helix database schema.
 * Use this class to select, insert, update and delete data in the persondate
 * table, as well as access related data in other tables.
 * 
 * If you need to extend the functionality of this class, code should be placed in a
 * class called PersondateExtension, and should extend the PersondateTable
 * class.  The custom code file should be in the helix/modules/Core folder
 * and should be called PersondateExtension.php
 * 
 * Object -> Persondate
 */
class PersondateTable extends Object {
	public $logSequence;
	public $fdate;
	public $tdate;
	...
	public function __construct($id=null, $value=null) {
		...
		if (isset($id)) {
			$condition = "{$db->le}persondate{$db->re}.{$db->le}id{$db->re}={$db->queryValue($id)}";
		}

		if (isset($condition)) {
			$query = implode(NL, array(
				"SELECT {$db->le}persondate{$db->re}.{$db->le}id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}value{$db->re}, {$db->le}persondate{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}mdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}cdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}deleted{$db->re}",
	...
	public static function snapshot($date, $id=null) {
		...
		if (isset($condition)) {
			$condition .= " AND ({$db->le}persondate{$db->re}.{$db->le}tdate{$db->re} IS NOT NULL AND {$db->le}persondate{$db->re}.{$db->le}fdate{$db->re}<={$db->queryValue($date)} AND {$db->queryValue($date)}<={$db->le}persondate{$db->re}.{$db->le}tdate{$db->re}) ";
			$query = implode(NL, array(
				"SELECT {$db->le}persondate{$db->re}.{$db->le}log_sequence{$db->re}, {$db->le}persondate{$db->re}.{$db->le}fdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}tdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}value{$db->re}, {$db->le}persondate{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}mdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}cdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}persondate{$db->re}",
				"WHERE {$condition}"
			));

			$query .= "ORDER BY {$db->le}persondate{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	public function insert() {
		...
		$db = Database::getInstance();
		$query = implode(NL, array(
			"INSERT INTO {$db->le}persondate{$db->re} (",
	...
	public function update() {
		...
		if ($this->isDirty()) {
			$db = Database::getInstance();
			$query = implode(NL, array(
				"UPDATE {$db->le}persondate{$db->re} SET",
	...
	protected function log() {
		...
		$db = Database::getInstance();
		$database = $db->getDatabase();
		$record = Persondate::select($db->encapsulate("persondate") . ".mdate as fdate,null as tdate,{$db->encapsulate("persondate")}.*",null,$db->encapsulate("persondate").".id={$this->id}")->first();
		$log = "{$database}_log";
		...
		$query = implode(NL, array(
			"INSERT INTO {$dbLog->le}persondate{$dbLog->re} (",
			$insertColumns,
			") VALUES (",
			$insertValues,
			")"
		...
		if ($logSequence){
			$query = implode(NL, array(
				...
				"FROM {$dbLog->le}persondate{$dbLog->re}",
				"WHERE {$dbLog->le}persondate{$dbLog->re}.{$dbLog->le}id{$dbLog->re}={$this->id}",
			...
			if ($dbLog->getRecord()) {
				...
				$query = implode(NL, array(
					"UPDATE {$dbLog->le}persondate{$dbLog->re}",
	...
	public function purge() {
		if (isset($this->_snapshot)) {return false;}
		$db = Database::getInstance();
		$query = "DELETE FROM " . $db->encapsulate("persondate") . " WHERE id={$db->queryValue($this->id)}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function deleteAll($where=null) {
		...
		$query = "UPDATE " . $db->encapsulate("persondate") . " SET deleted=1 {$condition}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function select($columns, $order=null, $where=null, $limit=null, $offset=0) {
		...
		$query = implode(NL, array(
			...
			"FROM {$db->le}persondate{$db->le}",

			"WHERE {$db->le}persondate{$db->re}.{$db->le}deleted{$db->re}=0" . (isset($where) ? " AND ({$where})" : ""),
	...
	public static function ids($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		return Persondate::select("{$db->le}persondate{$db->le}.id", $order, $where, $limit, $offset);
	}

	public static function objects($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		$objects = array();
		foreach (Persondate::select("{$db->le}persondate{$db->re}.{$db->le}id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}value{$db->re}, {$db->le}persondate{$db->re}.{$db->le}created_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}persondate{$db->re}.{$db->le}mdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}cdate{$db->re}, {$db->le}persondate{$db->re}.{$db->le}deleted{$db->re}", $order, $where, $limit, $offset) as $record) {
			$object = new Persondate();
				$object->id = $record["id"];
	...
	public static function search($query=null, $order=null, $where=null, $limit=null, $offset=0) {
		...
		return Persondate::objects($order, $where, $limit, $offset);
	}
	...
	public function getPersonPersondate($personId, $type="default") {
		return new PersonPersondate(null, $personId, $this->id, PersonPersondate::typeId($type));
	}
	...
	public function removePersonList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return PersonPersondate::deleteAll(null, $this->id, $type);
		}
	}
	...
	public function getPersonIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}person_persondate{$db->re} ON {$db->le}person_persondate{$db->re}.{$db->le}person_entity_id{$db->re}={$db->le}person{$db->re}.{$db->le}entity_id{$db->re} ",
			"	AND {$db->le}person_persondate{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}person{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}person_persondate{$db->re}.{$db->le}persondate_id{$db->re}={$db->queryValue($this->id)} ",
