 ...
 * 
 * This class represents the orgchart table in the Helix database schema.
 * Use this class to select, insert, update and delete data in the orgchart
 * table, as well as access related data in other tables.
 * 
 * If you need to extend the functionality of this class, code should be placed in a
 * class called OrgchartExtension, and should extend the OrgchartTable
 * class.  The custom code file should be in the helix/modules/Business folder
 * and should be called OrgchartExtension.php
 * 
 * Object -> Orgchart
 */
class OrgchartTable extends Object {
	public $logSequence;
	public $fdate;
	public $tdate;
	...
	public function __construct($id=null) {
		...
		if (isset($id)) {
			$condition = "{$db->le}orgchart{$db->re}.{$db->le}id{$db->re}={$db->queryValue($id)}";
		}

		if (isset($condition)) {
			$query = implode(NL, array(
				"SELECT {$db->le}orgchart{$db->re}.{$db->le}id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}name{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}description{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}mdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}cdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}deleted{$db->re}",
	...
	public static function snapshot($date, $id=null) {
		...
		if (isset($condition)) {
			$condition .= " AND ({$db->le}orgchart{$db->re}.{$db->le}tdate{$db->re} IS NOT NULL AND {$db->le}orgchart{$db->re}.{$db->le}fdate{$db->re}<={$db->queryValue($date)} AND {$db->queryValue($date)}<={$db->le}orgchart{$db->re}.{$db->le}tdate{$db->re}) ";
			$query = implode(NL, array(
				"SELECT {$db->le}orgchart{$db->re}.{$db->le}log_sequence{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}fdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}tdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}name{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}description{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}mdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}cdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}deleted{$db->re}",
				"FROM {$db->le}orgchart{$db->re}",
				"WHERE {$condition}"
			));

			$query .= "ORDER BY {$db->le}orgchart{$db->re}.log_sequence desc limit 1";
			$db->query($query);
	...
	public function insert() {
		...
		$db = Database::getInstance();
		$query = implode(NL, array(
			"INSERT INTO {$db->le}orgchart{$db->re} (",
	...
	public function update() {
		...
		if ($this->isDirty()) {
			$db = Database::getInstance();
			$query = implode(NL, array(
				"UPDATE {$db->le}orgchart{$db->re} SET",
	...
	protected function log() {
		...
		$db = Database::getInstance();
		$database = $db->getDatabase();
		$record = Orgchart::select($db->encapsulate("orgchart") . ".mdate as fdate,null as tdate,{$db->encapsulate("orgchart")}.*",null,$db->encapsulate("orgchart").".id={$this->id}")->first();
		$log = "{$database}_log";
		...
		$query = implode(NL, array(
			"INSERT INTO {$dbLog->le}orgchart{$dbLog->re} (",
			$insertColumns,
			") VALUES (",
			$insertValues,
			")"
		...
		if ($logSequence){
			$query = implode(NL, array(
				...
				"FROM {$dbLog->le}orgchart{$dbLog->re}",
				"WHERE {$dbLog->le}orgchart{$dbLog->re}.{$dbLog->le}id{$dbLog->re}={$this->id}",
			...
			if ($dbLog->getRecord()) {
				...
				$query = implode(NL, array(
					"UPDATE {$dbLog->le}orgchart{$dbLog->re}",
	...
	public function purge() {
		if (isset($this->_snapshot)) {return false;}
		$db = Database::getInstance();
		$query = "DELETE FROM " . $db->encapsulate("orgchart") . " WHERE id={$db->queryValue($this->id)}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function deleteAll($where=null) {
		...
		$query = "UPDATE " . $db->encapsulate("orgchart") . " SET deleted=1 {$condition}";
		$status = $db->query($query);
		return $status;
	}
	...
	public static function select($columns, $order=null, $where=null, $limit=null, $offset=0) {
		...
		$query = implode(NL, array(
			...
			"FROM {$db->le}orgchart{$db->le}",

			"WHERE {$db->le}orgchart{$db->re}.{$db->le}deleted{$db->re}=0" . (isset($where) ? " AND ({$where})" : ""),
	...
	public static function ids($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		return Orgchart::select("{$db->le}orgchart{$db->le}.id", $order, $where, $limit, $offset);
	}

	public static function objects($order=null, $where=null, $limit=null, $offset=0) {
		$db = Database::getInstance();
		$objects = array();
		foreach (Orgchart::select("{$db->le}orgchart{$db->re}.{$db->le}id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}name{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}description{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}updated_by_id{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}mdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}cdate{$db->re}, {$db->le}orgchart{$db->re}.{$db->le}deleted{$db->re}", $order, $where, $limit, $offset) as $record) {
			$object = new Orgchart();
				$object->id = $record["id"];
	...
	public static function search($query=null, $order=null, $where=null, $limit=null, $offset=0) {
		...
		foreach ($keywords as $keyword) {
			$clauses[] = "orgchart.name LIKE '%{$keyword}%' OR orgchart.description LIKE '%{$keyword}%'";
		}

		$search = implode(" AND ", $clauses);
		$where = isset($where) ? "{$where} AND ({$search})" : "({$search})";
		return Orgchart::objects($order, $where, $limit, $offset);
	}

	public function __toString() {
		return "Orgchart Object [" . alt($this->id, "null") . "]";
	}
	...
	public function getEntityOrgchart($entityId, $type="default") {
		return new EntityOrgchart(null, $entityId, $this->id, EntityOrgchart::typeId($type));
	}

	public function getOrgchartOrgposition($orgpositionId, $type="default") {
		return new OrgchartOrgposition(null, $this->id, $orgpositionId, OrgchartOrgposition::typeId($type));
	}
	...
	public function removeEntityList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return EntityOrgchart::deleteAll(null, $this->id, $type);
		}
	}
	...
	public function getEntityIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}entity_orgchart{$db->re} ON {$db->le}entity_orgchart{$db->re}.{$db->le}entity_id{$db->re}={$db->le}entity{$db->re}.{$db->le}id{$db->re} ",
			"	AND {$db->le}entity_orgchart{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}entity{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}entity_orgchart{$db->re}.{$db->le}orgchart_id{$db->re}={$db->queryValue($this->id)} ",
	...
	public function removeOrgpositionList($type=null) {
		if (isset($this->_snapshot)) {return false;}
		if ($this->id>0) {
			return OrgchartOrgposition::deleteAll($this->id, null, $type);
		}
	}
	...
	public function getOrgpositionIds($type=null, $order=null, $where=null, $limit=null, $offset=0, $primary=false) {
		...
		$query = implode(NL, array(
			...
			"INNER JOIN {$db->le}orgchart_orgposition{$db->re} ON {$db->le}orgchart_orgposition{$db->re}.{$db->le}orgposition_id{$db->re}={$db->le}orgposition{$db->re}.{$db->le}id{$db->re} ",
			"	AND {$db->le}orgchart_orgposition{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}orgposition{$db->re}.{$db->le}deleted{$db->re}=0 ",
			"	AND {$db->le}orgchart_orgposition{$db->re}.{$db->le}orgchart_id{$db->re}={$db->queryValue($this->id)} ",
